...
class Validator
{
    ...
    public function get_readable_errors($convert_to_string = false, $field_class = 'gump-field', $error_class = 'gump-error-message')
    {
        ...
        foreach ($this->errors as $e) {
            ...
            switch ($e['rule']) {
                ...
                case 'validate_alpha_numeric':
                    ...
                    break;
				case 'validate_wp_usernameExists':
                    $resp[] = "The <span class=\"$field_class\">$field</span> Username Already Exists";
                    break;
				case 'validate_wp_userExists':
                    $resp[] = "The <span class=\"$field_class\">$field</span> User Does Not Exists";
                    break;
				case 'validate_wp_EmailExists':
                    $resp[] = "The <span class=\"$field_class\">$field</span> Email Already Exists";
                    break;
                ...
                case 'validate_contains':
                    $resp[] = "The <span class=\"$field_class\">$field</span> field needs to contain valid parameters ";
                    break;
        ...
        if (!$convert_to_string) {
            ...
            $buffer = '';
            foreach ($resp as $s) {
                $buffer .= "<div class=\"$error_class\"><a href='#' class='close' data-dismiss='alert' aria-label='close'>&times;</a>$s</div>";
            }
        ...
        foreach ($this->errors as $e) {
            ...
            switch ($e['rule']) {
                ...
                case 'validate_alpha_numeric':
                    ...
                    break;
				case 'validate_wp_usernameExists':
                    $resp[$field] = "The $field Already Exists !";
                    break;
				case 'validate_wp_userExists':
                    $resp[$field] = "The $field Does Not Exists !";
                    break;
				case 'validate_wp_EmailExists':
                    $resp[$field] = "The $field Already Exists !";
                    break;
                ...
                case 'validate_valid_name':
                    ...
                    break;
                case 'validate_contains':
                    $resp[$field] = "The $field field needs to contain valid parameters ";
                    break;
                case 'validate_contains_list':
    ...
    */
	...
	/* Wordpress Check Username Exists */
	 protected function validate_wp_usernameExists($field, $input, $param = null)
    {
        if (!isset($input[$field]) || empty($input[$field])) {
            return;
        }
       ...
       if ( username_exists( $input[$field] ) )
		   {
				return array(
					'field' => $field,
					'value' => $input[$field],
	...
	/* Wordpress Email Exists */
	 protected function validate_wp_EmailExists($field, $input, $param = null)
    {
        if (!isset($input[$field]) || empty($input[$field])) {
            return;
        }		
		
       if ( email_exists( $input[$field] ) )
		   {
				return array(
					'field' => $field,
					'value' => $input[$field],
	...
	/* Wordpress Check User Exists */
	 protected function validate_wp_userExists($field, $input, $param = null)
    {
        if (!isset($input[$field]) || empty($input[$field])) {
            return;
        }		
		
       if ( ! get_userdata( $input[$field] ) )
		   {
				return array(
					'field' => $field,
					'value' => $input[$field],
